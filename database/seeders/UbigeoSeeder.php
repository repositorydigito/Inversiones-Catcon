...
class UbigeoSeeder extends Seeder
{
    ...
    {
        $this->command->info('Cargando datos de ubigeo...');

        // Obtener datos desde los métodos auxiliares
        $departamentos = $this->getDepartamentosData();
        $provincias = $this->getProvinciasData();
        $distritos = $this->getDistritosData();

        $this->command->info('Procesando y estructurando datos...');

        // Procesar datos al formato que necesita UbigeoService
        $ubigeoData = $this->processData($departamentos, $provincias, $distritos);

        // Guardar en config/ubigeo.php
        $configPath = config_path('ubigeo.php');
        $configContent = $this->generateConfigFile($ubigeoData);
        File::put($configPath, $configContent);

        $this->command->info("Datos cargados exitosamente en: {$configPath}");
        $this->command->info("Estadísticas:");
        $this->command->info("   • Departamentos: " . count($ubigeoData['departamentos']));
        $this->command->info("   • Provincias: " . array_sum(array_map('count', $ubigeoData['provincias'])));
        $this->command->info("   • Distritos: " . array_sum(array_map('count', $ubigeoData['distritos'])));
    }

    private function getDepartamentosData(): array
    {
        // 🎯 COPIAR Y PEGAR AQUÍ LOS DATOS DESDE:
        // https://raw.githubusercontent.com/ernestorivero/Ubigeo-Peru/master/php_arrays/departamentos.php
    ...
    private function getProvinciasData(): array
    {
        // 🎯 COPIAR Y PEGAR AQUÍ LOS DATOS DESDE:
        // https://raw.githubusercontent.com/ernestorivero/Ubigeo-Peru/master/php_arrays/provincias.php
    ...
    private function getDistritosData(): array
    {
        // 🎯 COPIAR Y PEGAR AQUÍ LOS DATOS DESDE:
        // https://raw.githubusercontent.com/ernestorivero/Ubigeo-Peru/master/php_arrays/distritos.php

        return [
            // LIMA - LIMA (Algunos distritos ejemplo)
    ...
    private function processData(array $departamentos, array $provincias, array $distritos): array
    {
