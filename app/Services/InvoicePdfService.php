...
class InvoicePdfService
{
    ...
    private function prepareInvoiceData(Invoice $invoice): array
    {
        ...
        if ($invoice->detraction) {
            $detractionAmount = $invoice->total * ($invoice->detraction_percentage / 100);
            $netPayable = $invoice->total - $detractionAmount;
            
            $detraction = [
                'percentage' => $invoice->detraction_percentage,
                'amount' => $detractionAmount,
                'net_payable' => $netPayable,
                'account' => $invoice->detraction_account ?? 'Cuenta BN: 00-000-000000'
            ];
        }
